...
const CommonPassword: React.FC<CommonPasswordProps> = ({
  ...
  eyeCloseIcon = DefaultEyeCloseIcon,
  inputClassName,
  labelClassName,
  buttonClassName,
}) => {
    ...
    <div className="flex flex-col space-y-2">
      ...
      <div className="relative">
        <input
          ...
          className={`w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300 ${inputClassName}`}
          placeholder={placeholder}
          value={password}
          onChange={(e) => setPassword(e.target.value)}
        />
        <button
          className={`absolute inset-y-0 right-2 flex items-center px-2 focus:outline-none ${buttonClassName}`}
